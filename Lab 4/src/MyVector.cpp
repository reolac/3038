...
//---------------------------------
{
	...
	float smallest = m_data.at(0);

	for (unsigned int i(1); i < getSize(); i++)
	{
		if (m_data.at(i) < smallest)
		{
			smallest = m_data.at(i);
		}
	}
...
//---------------------------------
{
	...
	float largest = m_data.at(0);

	for (unsigned int i(1); i < getSize(); i++)
	{
		if (m_data.at(i) > largest)
		{
			largest = m_data.at(i);
		}
	}
...
//---------------------------------
{
	...
	{
		variance += pow((m_data.at(i) - average),2);
	}
